...
program.version(version).description("Generate apps built with Nuxt, Vue or Vuetify for any API documented api").usage("entrypoint outputDirectory").option("-r, --resource [resourceName]", "Generate CRUD for the given resource").option("-g, --generator [generator]", 'The generator to use, one of "nuxt", "vue", "vuetify"').option("-t, --template-directory [templateDirectory]", "The templates directory base to use. Final directory will be ${templateDirectory}/${generator}", `${__dirname}/../templates/`).option("-s, --server-path [serverPath]", "Path to express server file to allow route dynamic addition (Next.js generator only)").parse(process.argv);
...
const entrypointWithSlash = entrypoint.endsWith("/") ? entrypoint : entrypoint + "/";
...
const parser = entrypointWithSlash => {
  ...
  if (program.username && program.password) {
    const encoded = Buffer.from(`${program.username}:${program.password}`).toString("base64");
...
parser(entrypointWithSlash).then(ret => {
  ret.api.resources.filter(({
    deprecated
  }) => !deprecated).filter(resource => {
    const nameLc = resource.name.toLowerCase();
    const titleLc = resource.title.toLowerCase();
    return null === resourceToGenerate || nameLc === resourceToGenerate || titleLc === resourceToGenerate;
  }).map(resource => {
    const filterDeprecated = list => list.filter(({
      deprecated
    }) => !deprecated);
