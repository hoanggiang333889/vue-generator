...
export default class NuxtGenerator extends BaseVueGenerator {
  constructor(params) {
    super(params);
    this.registerTemplates(`nuxt/`, [// components
    ...
    "pages/foos/new.vue", "pages/foos/index.vue", "pages/foos/_id.vue", // store
  ...
  generateFiles(api, resource, dir, params, module) {
    ...
    const lc = context.lc;
    [`${dir}/config`, `${dir}/error`, `${dir}/mixins`, `${dir}/services`, `${dir}/store`, `${dir}/utils`, `${dir}/validators`].forEach(dir => this.createDir(dir, false)); // error

    this.createFile("error/SubmissionError.js", `${dir}/error/SubmissionError.js`, {}, false); // mixins

    ["mixins/create.js", "mixins/list.js", "mixins/notification.js", "mixins/show.js", "mixins/update.js"].forEach(file => this.createFile(file, `${dir}/${file}`, context, false)); // stores

    this.createFile(`store/modules/notifications.js`, `${dir}/store/notifications.js`, {
      hydraPrefix: this.hydraPrefix
    }, false);
    this.createFile(`store/crud.js`, `${dir}/store/crud.js`, {
      hydraPrefix: this.hydraPrefix
    }, false); // validators
    ...
    "services/%s.js", // store
    "store/%s.js"].forEach(pattern => this.createFileFromPattern(pattern, dir, lc, context)); // components

    ["ActionCell.vue", "Alert.vue", "ConfirmDelete.vue", "DataFilter.vue", "InputDate.vue", "Loading.vue", "Toolbar.vue"].forEach(file => this.createFile(`components/${file}`, `${dir}/components/${file}`, context, false));
  }
