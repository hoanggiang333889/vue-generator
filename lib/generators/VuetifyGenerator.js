...
export default class extends BaseVueGenerator {
  constructor(params) {
    super(params);
    this.registerTemplates(`vuetify/`, [// components
...
`));
  ...
  generateFiles(api, resource, dir, params) {
    ...
    const lc = context.lc; // Create directories
    ...
    "router/%s.js", // views
    "views/%s/Create.vue", "views/%s/List.vue", "views/%s/Show.vue", "views/%s/Update.vue"].forEach(pattern => this.createFileFromPattern(pattern, dir, lc, context)); // components

    ["ActionCell.vue", "Breadcrumb.vue", "ConfirmDelete.vue", "DataFilter.vue", "InputDate.vue", "Loading.vue", "Snackbar.vue", "Toolbar.vue"].forEach(file => this.createFile(`components/${file}`, `${dir}/components/${file}`, context, false));
  }
